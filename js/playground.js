...
let customAlertOverlay, alertIcon, alertTitle, alertMessage, alertOkBtn, alertCancelBtn;
...
async function loadContent() {
    ...
    try {
        ...
        await showAlert('Failed to load content. Please check the data file.', 'Error', 'error');
        return;
    }
...
function isUserAdded(id, type) {
    const originalItems = type === 'poem' ? originalData.poems : originalData.quotes;
...
function renderPoems() {
    if (contentData.poems.length === 0) {
        poemsList.innerHTML = '<p style="text-align: center; color: #7f8c8d;">No poems yet. Add your first poem above!</p>';
        return;
    }

    poemsList.innerHTML = contentData.poems.map(poem => {
        const userAdded = isUserAdded(poem.id, 'poem');
        const badge = userAdded ? '<span class="user-badge">Your Creation</span>' : '<span class="original-badge">Original</span>';
        const buttons = userAdded
            ? `<button class="btn btn-edit" onclick="editPoem(${poem.id})">Edit</button>
               <button class="btn btn-danger" onclick="deletePoem(${poem.id})">Delete</button>`
            : '<span class="read-only-label">Read-Only</span>';

        return `
            <div class="item-card ${poem.category} ${userAdded ? 'user-added' : 'original-content'}">
                ...
                <div class="item-meta">
                    <strong>${poem.category}</strong> | ${poem.author} | ${formatDate(poem.date)}
                </div>
            </div>
        `;
    }).join('');
...
function renderQuotes() {
    if (contentData.quotes.length === 0) {
        quotesList.innerHTML = '<p style="text-align: center; color: #7f8c8d;">No quotes yet. Add your first quote above!</p>';
        return;
    }

    quotesList.innerHTML = contentData.quotes.map(quote => {
        const userAdded = isUserAdded(quote.id, 'quote');
        const badge = userAdded ? '<span class="user-badge">Your Creation</span>' : '<span class="original-badge">Original</span>';
        const buttons = userAdded
            ? `<button class="btn btn-edit" onclick="editQuote(${quote.id})">Edit</button>
               <button class="btn btn-danger" onclick="deleteQuote(${quote.id})">Delete</button>`
            : '<span class="read-only-label">Read-Only</span>';

        return `
            <div class="item-card ${quote.category} ${userAdded ? 'user-added' : 'original-content'}">
                <div class="item-header">
                ...
                <div class="item-meta">
                    <strong>${quote.category}</strong> | ${quote.author} | ${formatDate(quote.date)}
                </div>
            </div>
        `;
    }).join('');
...
async function downloadJSON() {
    ...
    await showAlert(
        'JSON file downloaded! Replace data/content.json with this file to update your website.',
        'Download Complete',
        'success'
    );
}
